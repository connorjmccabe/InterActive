...
focal.seq <- sort(c(-1*sd(data[,foc],na.rm=T),
      ...
      if(input$cat==FALSE & values$co <= max(focal.seq) & values$co >= min(focal.seq)){
      ...
      if(input$cat==FALSE & values$co <= max(df.staticplot$focal.seq) & values$co >= min(df.staticplot$focal.seq))
      {

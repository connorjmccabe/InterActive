...
server<-function(input, output, session) {
  ...
  observeEvent(input$go, { #Once the "go" button is hit, InterActive looks at all the ui input and runs the model.
    ...
    output$modplot <- renderPlot({
      ...
      dfcoefs<-data #copy data for creating summary table of coefficients. Reflects scaling of X, Z, and Y.
      ...
      values$m<-plotelements$model<-lm(values$form,dfcoefs, na.action = na.omit)
      ...
      #Note: lm.beta will not produce correct beta value for the interaction coefficient, so this is computed on it's own below.
      
      # beta <- b * sx/sy
      
      resultstable[paste0(foc,":",mod),"Beta"]<-round(m$coefficients[paste0(foc,":",mod)]*sd(m$model[,foc]*m$model[,mod],na.rm=T)/sd(m$model[,dv],na.rm=T),3)
...
focal.seq <- sort(c(-1*sd(data[,foc],na.rm=T),
      ...
      if (input$scalez == "raw"){
      sm1<-mean(dftrue[,mod], na.rm=TRUE) + as.numeric(input$sm1)*(sd(dftrue[,mod], na.rm=TRUE))
      sm2<-mean(dftrue[,mod], na.rm=TRUE) + as.numeric(input$sm2)*(sd(dftrue[,mod], na.rm=TRUE))
      sm3<-mean(dftrue[,mod], na.rm=TRUE) + as.numeric(input$sm3)*(sd(dftrue[,mod], na.rm=TRUE))
      sm4<-mean(dftrue[,mod], na.rm=TRUE) + as.numeric(input$sm4)*(sd(dftrue[,mod], na.rm=TRUE))
      sm5<-mean(dftrue[,mod], na.rm=TRUE) + as.numeric(input$sm5)*(sd(dftrue[,mod], na.rm=TRUE))
      }
      if (input$scalez == "cen"){
        sm1<-as.numeric(input$sm1)*(sd(scale(dftrue[,mod],scale=F,center=T), na.rm=TRUE))
        sm2<-as.numeric(input$sm2)*(sd(scale(dftrue[,mod],scale=F,center=T), na.rm=TRUE))
        sm3<-as.numeric(input$sm3)*(sd(scale(dftrue[,mod],scale=F,center=T), na.rm=TRUE))
        sm4<-as.numeric(input$sm4)*(sd(scale(dftrue[,mod],scale=F,center=T), na.rm=TRUE))
        sm5<-as.numeric(input$sm5)*(sd(scale(dftrue[,mod],scale=F,center=T), na.rm=TRUE))
      }
      if (input$scalez == "std"){
        sm1<-as.numeric(input$sm1)*(sd(scale(dftrue[,mod],scale=T,center=T), na.rm=TRUE))
        sm2<-as.numeric(input$sm2)*(sd(scale(dftrue[,mod],scale=T,center=T), na.rm=TRUE))
        sm3<-as.numeric(input$sm3)*(sd(scale(dftrue[,mod],scale=T,center=T), na.rm=TRUE))
        sm4<-as.numeric(input$sm4)*(sd(scale(dftrue[,mod],scale=T,center=T), na.rm=TRUE))
        sm5<-as.numeric(input$sm5)*(sd(scale(dftrue[,mod],scale=T,center=T), na.rm=TRUE))
        #NOTE: the sd of the scaled moderator will be 1, so (e.g.) sm1<-as.numeric(input$sm1)*1.
      }
      ...
      mod.input <- c(as.numeric(input$sm1),
      ...
      values$matxi<-plotelements$matxi<-matxi
      values$Xmat<-plotelements$Xmat<-Xmat
      ...
      for (k in 1:length(mod.seq)){
        ...
        if (input$scalez == "raw"){data$s.mod<-dftrue[,mod]-mod.seq[k]}
        if (input$scalez == "cen"){data$s.mod<-scale(dftrue[,mod],center=T,scale=F)-mod.seq[k]}
        if (input$scalez == "std"){data$s.mod<-scale(dftrue[,mod],scale=T)-mod.seq[k]}
        ...
        else(level[[k]]<-paste0(mod.input[k]," SD (PTCL = ",round(percentiles[k],2),")",leveltemp))
        # else(level[[k]]<-paste0(mod.seq[k]/sd(data[,mod],na.rm=TRUE)," SD (PTCL = ",round(percentiles[k],2),")",leveltemp))
      
      #   else{
      #     if (input$scalez == "raw"){
      #       level[[k]]<-paste0(round(mod.seq[k],2)," (PTCL = ",round(percentiles[k],2),")",leveltemp)
      #       }
      #     if (input$scalez == "cen" | input$scalez == "std"){
      #         level[[k]]<-paste0(mod.seq[k]/sd(data[,mod],na.rm=TRUE)," SD (PTCL = ",round(percentiles[k],2),")",leveltemp)
      #       }
      # }
      }
  ...
  output$cite<-renderUI({
    ...
    str2 <- paste0("McCabe, C., Kim, D., & King, K. (2018). Improving Present Practices in the Visual Display of Interactions. Advances in Methods and Practices in Psychological Science, 1(2), 147-165.")
